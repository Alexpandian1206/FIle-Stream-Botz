...
async def batch_page(message_id):
    ...
    for file_name, link in links_with_names:
        buttons_html += f'<form action="{link}" method="get"><button style="font-size: 20px; background-color: rgba(255, 255, 255, 0.2); border-radius: 10px; backdrop-filter: blur(10px);" class="button" type="submit">{file_name}</button></form>\n<br><p>&nbsp</p>'
